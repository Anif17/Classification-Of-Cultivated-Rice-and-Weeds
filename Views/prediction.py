...
# Define a threshold for uncertainty (you can adjust this value)
confidence_threshold = 0.7  # Example: below 60% confidence is considered uncertain
...
if uploaded_file is not None:
    ...
    # Get the maximum confidence score and the corresponding predicted class
    max_confidence = np.max(confidence_scores)

    # Check if the maximum confidence score is below the threshold
    ...
    # Prediction results
    duration = end_time - start_time
    st.session_state.predicted_labels.append(predicted_class)
    ...
    with col2:
        ...
        if predicted_class != "Uncertain":
            with st.expander("View Confidence Scores", expanded=False):  # Set expanded=True to show by default
                ...
                st.write(f"The reason for predicting class **{predicted_class}** is because it shows the highest confidence score of **{confidence_scores[0][predicted.item()]:.2%}**.")
        ...
        with st.expander("Prevention and Control Tips", expanded=True): # Set expanded=True to show by default
            ...
            if predicted_class != "Uncertain":
                ...
                st.write("### Uncertain Prediction")
                st.write("The model was uncertain about the classification, as the confidence score for all classes was too low.")
                st.write("Please try uploading another image for a more accurate prediction.")
